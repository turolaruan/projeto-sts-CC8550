...
@pytest.mark.asyncio
async def test_register_transaction_requires_account(monkeypatch, capsys) -> None:
    user = build_user()
    container = SimpleNamespace(
        transaction_service=SimpleNamespace(create_transaction=AsyncMock()),
        user_service=SimpleNamespace(list_users=AsyncMock(return_value=[user])),
        account_service=SimpleNamespace(list_accounts=AsyncMock(return_value=[])),
        category_service=SimpleNamespace(list_categories=AsyncMock(return_value=[])),
    )
    ...
    await cli.register_transaction(container)
    output = capsys.readouterr().out
    assert "Cadastre uma conta primeiro." in output


@pytest.mark.asyncio
async def test_register_transaction_requires_category(monkeypatch, capsys) -> None:
    user = build_user()
    account = build_account(user_id=user.id)
    container = SimpleNamespace(
        transaction_service=SimpleNamespace(create_transaction=AsyncMock()),
        user_service=SimpleNamespace(list_users=AsyncMock(return_value=[user])),
        account_service=SimpleNamespace(list_accounts=AsyncMock(return_value=[account])),
        category_service=SimpleNamespace(list_categories=AsyncMock(return_value=[])),
    )
    monkeypatch.setattr("builtins.input", InputFeeder(["1", "1"]))
    monkeypatch.setattr("src.interface.cli.pause", lambda: None)

    await cli.register_transaction(container)
    output = capsys.readouterr().out
    assert "Cadastre uma categoria primeiro." in output
...
@pytest.mark.asyncio
async def test_manage_budgets_requires_expense_category(monkeypatch, capsys) -> None:
    monkeypatch.setattr("src.interface.cli.pause", lambda: None)
    user = build_user()
    service = SimpleNamespace(
    ...
    monkeypatch.setattr("builtins.input", InputFeeder(["2", "1", "0"]))
    await cli.manage_budgets(container)
    output = capsys.readouterr().out
    assert "Cadastre uma categoria de despesa" in output
...
@pytest.mark.asyncio
async def test_show_report_requires_user(monkeypatch, capsys) -> None:
    monkeypatch.setattr("src.interface.cli.pause", lambda: None)
    container = SimpleNamespace(
        user_service=SimpleNamespace(list_users=AsyncMock(return_value=[])),
        report_service=SimpleNamespace(monthly_summary=AsyncMock()),
    )
    await cli.show_report(container)
    output = capsys.readouterr().out
    assert "Cadastre um usuário primeiro." in output
    container.report_service.monthly_summary.assert_not_awaited()
