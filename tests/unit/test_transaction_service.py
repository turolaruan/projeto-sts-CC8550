...
@pytest.mark.asyncio
async def test_ensure_budget_allows_skips_non_expense_transactions() -> None:
    ctx = _make_service()
    ctx.budget_repository.list.reset_mock()
    ctx.repository.sum_for_category_period.reset_mock()
    transaction = build_transaction(
        id=TRANSACTION_ID,
        user_id=USER_ID,
        account_id=ACCOUNT_ID,
    ...
    await ctx.service._ensure_budget_allows(transaction, exclude_amount=Decimal("0"))

    ctx.budget_repository.list.assert_not_awaited()
    ctx.repository.sum_for_category_period.assert_not_awaited()


@pytest.mark.asyncio
async def test_ensure_budget_allows_passes_when_within_limit() -> None:
    ctx = _make_service()
    budget = build_budget(
        user_id=USER_ID,
        category_id=CATEGORY_ID,
    ...
    ctx.budget_repository.list.return_value = [budget]
    ctx.repository.sum_for_category_period.return_value = Decimal("120.00")
    transaction = build_transaction(
        id=TRANSACTION_ID,
        user_id=USER_ID,
        account_id=ACCOUNT_ID,
    ...
    await ctx.service._ensure_budget_allows(transaction, exclude_amount=Decimal("40.00"))

    ctx.repository.sum_for_category_period.assert_awaited_once()
