...
async def test_users_controller_error_branches() -> None:
    ...
    service.create_user.side_effect = ValidationAppError("invalid data")
    with pytest.raises(HTTPException) as excinfo:
        await users_ctrl.create_user(payload, service=service)
    ...
    service.update_user.side_effect = EntityNotFoundError("missing")
    with pytest.raises(HTTPException) as excinfo:
        await users_ctrl.update_user("user", UserUpdate(name="Novo"), service=service)
...
async def test_budgets_controller_error_paths() -> None:
    ...
    service.get_budget = AsyncMock(side_effect=EntityNotFoundError("missing"))
    with pytest.raises(HTTPException) as excinfo:
        await budgets_ctrl.get_budget("budget", service=service)
    assert excinfo.value.status_code == 404
...
async def test_transactions_controller_error_paths() -> None:
    ...
    service.delete_transaction = AsyncMock(side_effect=EntityNotFoundError("missing"))
    with pytest.raises(HTTPException):
        await transactions_ctrl.delete_transaction("txn", service=service)
